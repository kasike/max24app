...
export default function Employees({ 
  ...
  const resolveShiftState = (rawShift: string) => {
    if (!rawShift) {
      setShiftSelect('Mañana (08:00 - 16:00)');
      setCustomShiftText('');
    } else if (PRESET_SHIFTS.includes(rawShift)) {
      setShiftSelect(rawShift);
      setCustomShiftText('');
    } else if (rawShift === 'Mañana') {
      setShiftSelect('Mañana (08:00 - 16:00)');
      setCustomShiftText('');
    } else if (rawShift === 'Tarde') {
      setShiftSelect('Tarde (16:00 - 00:00)');
      setCustomShiftText('');
    } else if (rawShift === 'Noche') {
      setShiftSelect('Noche (00:00 - 08:00)');
      setCustomShiftText('');
    } else if (rawShift === 'Rotativo') {
      setShiftSelect('Rotativo completo');
      setCustomShiftText('');
    } else {
      setShiftSelect('Personalizado');
      setCustomShiftText(rawShift);
    }
  };
  ...
  const openCreateForm = () => {
    ...
    setEmail('');
    setRole('Cajero');
    resolveShiftState('Mañana (08:00 - 16:00)');
    setStatus('Activo');
    setUsername('');
  ...
  const submitForm = (e: React.FormEvent) => {
    ...
    const finalShift = shiftSelect === 'Personalizado'
      ? (customShiftText.trim() || 'Horario Personalizado')
      : shiftSelect;
                ...
                <div>
                  ...
                  <div className="space-y-2 mt-4 border-t border-slate-100 pt-3.5 text-left text-xs text-slate-505">
                    ...
                    <div className="flex items-center justify-between text-[11px] text-slate-500 bg-slate-50/80 p-2 rounded-xl border border-slate-100">
                      <p className="flex items-center gap-2 truncate pr-1">
                        <Clock className="w-3.5 h-3.5 text-orange-500 shrink-0" />
                        <span className="truncate">Turno: <strong className="text-slate-900 font-extrabold">{emp.shift || 'Sin configurar'}</strong></span>
                      </p>
                      <button
                        type="button"
                        onClick={() => openEditForm(emp)}
                        className="text-[9.5px] text-orange-700 hover:text-orange-900 font-extrabold bg-orange-100/80 hover:bg-orange-200/90 px-2 py-0.5 rounded-lg border border-orange-200 transition-colors shrink-0 cursor-pointer"
                        title="Modificar horario de este empleado"
                      >
                        ✏️ Modificar
                      </button>
                    </div>
            ...
            <form onSubmit={handleManualClockInSubmit} className="space-y-4">
              ...
              <div>
                ...
                <input
                  ...
                  placeholder="ej. Mañana (08:00 - 16:00) o 09:00 - 17:00"
                  className="w-full px-3.5 py-2.5 bg-slate-50 border border-slate-200 rounded-xl text-xs font-bold text-slate-800 focus:outline-hidden"
                />
                <div className="flex gap-1.5 mt-1.5 flex-wrap">
                  {Array.from(new Set([showClockInModal.shift, 'Mañana (08:00 - 16:00)', 'Tarde (16:00 - 00:00)', 'Noche (00:00 - 08:00)'])).filter(Boolean).map((s, idx) => (
                    <button
                      key={idx}
                      type="button"
                      onClick={() => setManualShift(s)}
                      className="text-[9.5px] bg-slate-100 hover:bg-slate-200 text-slate-700 font-semibold px-2 py-0.5 rounded-md border border-slate-200 cursor-pointer"
                    >
                      {s}
                    </button>
                  ))}
          ...
          <div className="bg-white rounded-3xl max-w-lg w-full border border-slate-200 shadow-2xl p-6 relative max-h-[90vh] overflow-y-auto">
            ...
            <form onSubmit={submitForm} className="space-y-4">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-3.5">
                ...
                {/* Shift selection & Custom Schedule Editor */}
                <div className="md:col-span-2 p-3.5 bg-slate-50/80 border border-slate-200/80 rounded-2xl space-y-3">
                  <div className="flex items-center justify-between">
                    ...
                    <span className="text-[10px] text-orange-600 font-bold bg-orange-50 px-2 py-0.5 rounded-md border border-orange-200/60">
                      {shiftSelect === 'Personalizado' ? '✏️ Modo Personalizado' : '📋 Turno Estándar'}
                    </span>
                  </div>
                  ...
                  <select
                    ...
                    className="w-full px-3.5 py-2.5 bg-white border border-slate-200 rounded-xl text-xs font-semibold focus:outline-hidden cursor-pointer shadow-2xs"
                  >
                    <option value="Mañana (08:00 - 16:00)">Mañana (08:00 - 16:00)</option>
                    <option value="Tarde (16:00 - 00:00)">Tarde (16:00 - 00:00)</option>
                    <option value="Noche (00:00 - 08:00)">Noche (00:00 - 08:00)</option>
                    <option value="Rotativo completo">Rotativo completo</option>
                    <option value="Personalizado">✏️ Configurar Horario Personalizado...</option>
                  </select>

                  {shiftSelect === 'Personalizado' && (
                    <div className="space-y-2 pt-1 animate-fade-in">
                      <label className="block text-[9.5px] font-bold text-slate-600 uppercase tracking-wider">
                        Escriba el Horario Específico o Jornada de Trabajo:
                      </label>
                      <input
                        type="text"
                        placeholder="ej. 09:00 - 17:00 / Lunes a Viernes 08:30 a 16:30"
                        value={customShiftText}
                        onChange={(e) => setCustomShiftText(e.target.value)}
                        className="w-full px-3.5 py-2.5 bg-white border border-orange-300 rounded-xl text-xs font-bold text-slate-900 focus:outline-hidden focus:ring-2 focus:ring-orange-500/20 shadow-2xs"
                        required={shiftSelect === 'Personalizado'}
                      />
                      <div className="flex items-center gap-1.5 flex-wrap pt-1">
                        <span className="text-[9px] text-slate-400 font-semibold">Sugerencias rápidas:</span>
                        {['09:00 - 17:00', '10:00 - 18:00', '12:00 - 20:00', '14:00 - 22:00', '08:00 - 12:00 / 16:00 - 20:00'].map((preset) => (
                          <button
                            key={preset}
                            type="button"
                            onClick={() => setCustomShiftText(preset)}
                            className="text-[9.5px] bg-orange-100/70 hover:bg-orange-200/80 text-orange-950 font-bold px-2 py-0.5 rounded-md border border-orange-200 transition-colors cursor-pointer"
                          >
                            + {preset}
                          </button>
                        ))}
