...
### Corregido
- **Mejora de Contraste y Estado Activo en Pestañas de Inicio de Sesión (`Login.tsx`)**: Se solucionó la falta de legibilidad e identificación del portal y rol activo (Dueño/Comercio vs Cajero/Empleado). Se aplicó el color primario de la marca MAX24 (`bg-orange-500` con texto blanco puro `text-white font-extrabold`), elevación con sombra suave (`shadow-md shadow-orange-500/30`), borde delimitador y atributos ARIA de accesibilidad (`role="tab"` y `aria-selected`), haciendo resaltar de forma inconfundible la pestaña seleccionada.
